...
class MGANLayer(nn.Module):
    def __init__(self,hidden_dim,adjacency_matrix,num_heads=4):
        super().__init__()
        self.heads=nn.ModuleList([TGAHead(hidden_dim,adjacency_matrix) for _ in range(num_heads)])
...
class PortfolioHead(nn.Module):
    ...
    def forward(self, x):
        ...
        return weights
class CombinedMGANPortfolio(nn.Module):
    def __init__(self, sector_adj, industry_adj, theme_adj, num_assets=15, hidden_dim=64, num_heads=4):
        ...
        # 4 graphs (sector, industry, theme, dynamic) + E_stock = 5 * hidden_dim
        self.portfolio_head = PortfolioHead(hidden_dim=hidden_dim, num_graphs=4)
        # 3 static graphs + E_stock = 4 * hidden_dim (Dynamic is added as residual)
        self.portfolio_head = PortfolioHead(hidden_dim=hidden_dim, num_graphs=3)
    def forward(self, x, dynamic_adj):
        E_stock, stock_attention = self.encoder(x)
        
        # Parallel fusion: all graph layers receive the base E_stock
        # Sequential chain for static graphs
        E_sector, _ = self.sector_graph(E_stock)
        E_industry, _ = self.industry_graph(E_stock)
        E_theme, _ = self.theme_graph(E_stock)
        E_industry, _ = self.industry_graph(E_sector)
        E_theme, _ = self.theme_graph(E_industry)
        
        # Parallel branch for dynamic graph (which is based on stock momentum)
        E_dynamic, _ = self.dynamic_graph(E_stock, dynamic_adj)
        # Residual fusion: add the dynamic graph's features to the final static theme features
        # This keeps the dimensionality identical to the static baseline (preventing overfitting)
        fused_embeddings = torch.cat([
            E_stock, E_sector, E_industry, E_theme + E_dynamic
        ], dim=-1)
...
num_epochs    = 50
best_val      = -np.inf
best_path     = "/Users/007vd/Downloads/DAU/Split_Graph_dl/src/combined/best_combined_model.pt"
train_sharpes = []
val_sharpes   = []
...
results = []
results.append(compute_all_metrics(test_port_r,                               "Combined MGAN (ours)"))
...
axes[1].legend()
plt.tight_layout()
plt.savefig("/Users/007vd/Downloads/DAU/Split_Graph_dl/src/combined/training_curves_combined.png", dpi=150)
plt.close()
fig2, axes2 = plt.subplots(1, 2, figsize=(16, 5))
colors = ["steelblue" if s != "Combined MGAN (ours)" else "darkorange" for s in results_df.index]
...
plt.tight_layout()
plt.savefig("/Users/007vd/Downloads/DAU/Split_Graph_dl/src/combined/baseline_comparison_combined.png", dpi=150)
plt.close()
fig3, ax3 = plt.subplots(figsize=(14, 6))
wealth_mgan = torch.cumprod(1 + test_port_r, dim=0).numpy()
ax3.plot(wealth_mgan, label="Combined MGAN (ours)", linewidth=2, color="darkorange")
static_baselines = [
    ("Equal Weight",        w_ew),
    ("MV Max-Sharpe",       w_mv),
...
plt.tight_layout()
plt.savefig("/Users/007vd/Downloads/DAU/Split_Graph_dl/src/combined/cumulative_wealth_combined.png", dpi=150)
plt.close()
